...
ssh-set:
	...
	ssh-add ~/.ssh/$(VMNAME)_ed25519
	ssh-keyscan -p $(SSH_HOST) localhost >> ~/.ssh/known_hosts
# ssh-copy-id -p $(SSH_HOST) -i ~/.ssh/$(VMNAME)_ed25519 $(USER)@localhost
# ssh -v -i ~/.ssh/$(VMNAME)_ed25519 -p $(SSH_HOST) $(USER)@localhost


ssh-config:
	@./title.sh "adding $(VMNAME) to ssh config"
	...
	echo "    IdentityFile ~/.ssh/$(VMNAME)_ed25519" >> ~/.ssh/config && \
...
login:
	...
	scp -i ~/.ssh/$(VMNAME)_ed25519 -P $(SSH_HOST) templates/login.sh $(USER)@localhost:/home/$(USER)/login.sh
	ssh -t $(VMNAME) "sudo chmod +x /home/$(USER)/login.sh"
	ssh -t $(VMNAME) "sudo /home/$(USER)/login.sh"

pam:
	@./title.sh "updating password security"
	scp -i ~/.ssh/$(VMNAME)_ed25519 -P $(SSH_HOST) templates/pam.sh $(USER)@localhost:/home/$(USER)/pam.sh
